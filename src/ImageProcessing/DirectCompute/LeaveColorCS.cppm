...
#include <QByteArray>
#include <QString>
module ImageProcessing.LeaveColorCS;
...
namespace ArtifactCore
{
	...
	~Impl()
	{
	 ...
	 void readFromByteCode(const QString& csoFilePath)
	 {
	  ...
	  ShaderCI.Desc.Name = "LeaveColor_CS";
	 ...
	 RefCntAutoPtr<IShader> pComputeShader;
	  ...
	  PSOCreateInfo.PSODesc.Name = "LeaveColorCS_PSO";
	 ...
	 void Process(cv::Mat& mat, float keyR, float keyG, float keyB, float keyA, float tolerance, float softness, float desaturateAmount)
	 {
	  TextureDesc InputTexDesc;
	  InputTexDesc.Name = "Input LeaveColor Texture";
	  ...
	  OutputTexDesc.Name = "Output LeaveColor Texture";
	  ...
	  struct { float key[4]; float tolerance; float softness; float desaturateAmount; float pad; } params = { keyR, keyG, keyB, keyA, tolerance, softness, desaturateAmount, 0 };
	...
	LeaveColorCS::LeaveColorCS(IRenderDevice* pDevice, IDeviceContext* pContext)
		: impl_(new Impl(pDevice, pContext))
	{
	}

	LeaveColorCS::~LeaveColorCS()
	{
	}

	 void LeaveColorCS::Process(cv::Mat& mat, float keyR, float keyG, float keyB, float keyA, float tolerance, float softness, float desaturateAmount)
	 {
	  impl_->Process(mat, keyR, keyG, keyB, keyA, tolerance, softness, desaturateAmount);
	 }

	 void LeaveColorCS::loadShaderBinaryFile(const QString& path)
	 {
	 }

	 void LeaveColorCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename)
	 {
	  if (!baseDir.exists()) {
