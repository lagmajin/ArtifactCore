...
#include <QByteArray>
#include <QString>
module ImageProcessing.BroadcastColorsCS;
...
namespace ArtifactCore
{
	...
	~Impl()
	{
	 ...
	 void readFromByteCode(const QString& csoFilePath)
	 {
	  ...
	  ShaderCI.Desc.Name = "BroadcastColors_CS";
	 ...
	 RefCntAutoPtr<IShader> pComputeShader;
	  ...
	  PSOCreateInfo.PSODesc.Name = "BroadcastColorsCS_PSO";
	 ...
	 void Process(cv::Mat& mat, int standard, float reduceSaturation)
	 {
	  ...
	  InputTexDesc.Name = "Input BroadcastColors Texture";
	  ...
	  OutputTexDesc.Name = "Output BroadcastColors Texture";
	  ...
	  struct { int standard; float reduceSaturation; float pad[2]; } params = { standard, reduceSaturation, 0, 0 };
	...
	BroadcastColorsCS::BroadcastColorsCS(IRenderDevice* pDevice, IDeviceContext* pContext)
	...
	BroadcastColorsCS::~BroadcastColorsCS()
	{
	 ...
	 {
	  impl_->Process(mat, standard, reduceSaturation);
	 }

	 void BroadcastColorsCS::loadShaderBinaryFile(const QString& path)
	 {
	 }

	 void BroadcastColorsCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename)
	 {
	  if (!baseDir.exists()) {
