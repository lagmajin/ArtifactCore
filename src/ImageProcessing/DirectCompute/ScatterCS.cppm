...
#include <QByteArray>
#include <QString>
module ImageProcessing.ScatterCS;
...
namespace ArtifactCore
{
	...
	~Impl()
	{
	 ...
	 RefCntAutoPtr<IShader> pComputeShader;
	  ...
	  PSOCreateInfo.PSODesc.Name = "ScatterCS_PSO";
	 ...
	 void Process(cv::Mat& mat, float amount, int seed)
	 {
	  ...
	  InputTexDesc.Name = "Input Scatter Texture";
	  ...
	  OutputTexDesc.Name = "Output Scatter Texture";
	  ...
	  struct { float amount; int seed; float pad[2]; } params = { amount, seed, 0, 0 };
	...
	ScatterCS::ScatterCS(IRenderDevice* pDevice, IDeviceContext* pContext)
	...
	ScatterCS::~ScatterCS()
	{
	 ...
	 {
	  impl_->Process(mat, amount, seed);
	 }

	 void ScatterCS::loadShaderBinaryFile(const QString& path)
	 {
	 }

	 void ScatterCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename)
	 {
	  if (!baseDir.exists()) {
