...
module ImageProcessing.LumaKeyCS;
...
public:
    ...
    void readFromByteCode(const QString& csoFilePath) {
        ...
        PSOCreateInfo.PSODesc.Name = "LumaKeyPSO";
    ...
    void Process(cv::Mat& mat, float lowThresh, float highThresh, float softness) {
        ...
        struct Params { float low; float high; float softness; float pad; };
        Params params = { lowThresh, highThresh, softness, 0 };
...
LumaKeyCS::LumaKeyCS(IRenderDevice* pDevice, IDeviceContext* pContext)
    : impl_(new Impl(pDevice, pContext)) {}
LumaKeyCS::~LumaKeyCS() { delete impl_; }

void LumaKeyCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename) {
...
void LumaKeyCS::Process(cv::Mat& mat, float lowThresh, float highThresh, float softness) {
    impl_->Process(mat, lowThresh, highThresh, softness);
}
