...
#include <QByteArray>
#include <QString>
module ImageProcessing.EmbossCS;
...
namespace ArtifactCore
{
	...
	~Impl()
	{
	 ...
	 RefCntAutoPtr<IShader> pComputeShader;
	  ...
	  PSOCreateInfo.PSODesc.Name = "EmbossCS_PSO";
	 ...
	 void Process(cv::Mat& mat, float intensity, float angle)
	 {
	  ...
	  InputTexDesc.Name = "Input Emboss Texture";
	  ...
	  OutputTexDesc.Name = "Output Emboss Texture";
	  ...
	  struct { float intensity; float angle; float pad[2]; } params = { intensity, angle, 0, 0 };
	...
	EmbossCS::EmbossCS(IRenderDevice* pDevice, IDeviceContext* pContext)
	...
	EmbossCS::~EmbossCS()
	{
	 ...
	 {
	  impl_->Process(mat, intensity, angle);
	 }

	 void EmbossCS::loadShaderBinaryFile(const QString& path)
	 {
	 }

	 void EmbossCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename)
	 {
	  if (!baseDir.exists()) {
