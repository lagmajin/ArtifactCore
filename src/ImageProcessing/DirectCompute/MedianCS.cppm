...
module ImageProcessing.MedianCS;
...
public:
    ...
    void readFromByteCode(const QString& csoFilePath) {
        ...
        PSOCreateInfo.PSODesc.Name = "MedianPSO";
    ...
    void Process(cv::Mat& mat, int radius) {
        ...
        struct Params { int radius; int pad[3]; };
        Params params = { radius, 0, 0, 0 };
...
MedianCS::MedianCS(IRenderDevice* pDevice, IDeviceContext* pContext)
    : impl_(new Impl(pDevice, pContext)) {}
MedianCS::~MedianCS() { delete impl_; }

void MedianCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename) {
...
void MedianCS::Process(cv::Mat& mat, int radius) {
    impl_->Process(mat, radius);
}
