...
#include <QByteArray>
#include <QString>
module ImageProcessing.ChromaKeyCS;
...
namespace ArtifactCore
{
	...
	~Impl()
	{
	 ...
	 void readFromByteCode(const QString& csoFilePath)
	 {
	  ...
	  ShaderCI.Desc.Name = "ChromaKey_CS";
	 ...
	 RefCntAutoPtr<IShader> pComputeShader;
	  ...
	  PSOCreateInfo.PSODesc.Name = "ChromaKeyCS_PSO";
	 ...
	 void Process(cv::Mat& mat, float keyR, float keyG, float keyB, float keyA, float tolerance, float softness)
	 {
	  ...
	  InputTexDesc.Name = "Input ChromaKey Texture";
	  ...
	  OutputTexDesc.Name = "Output ChromaKey Texture";
	  ...
	  struct { float key[4]; float tolerance; float softness; float pad[2]; } params = { keyR, keyG, keyB, keyA, tolerance, softness, 0, 0 };
	...
	ChromaKeyCS::ChromaKeyCS(IRenderDevice* pDevice, IDeviceContext* pContext)
	...
	ChromaKeyCS::~ChromaKeyCS()
	{
	 ...
	 {
	  impl_->Process(mat, keyR, keyG, keyB, keyA, tolerance, softness);
	 }

	 void ChromaKeyCS::loadShaderBinaryFile(const QString& path)
	 {
	 }

	 void ChromaKeyCS::loadShaderBinaryFromDirectory(const QDir& baseDir, const QString& filename)
	 {
	  if (!baseDir.exists()) {
