...
bool FFmpegVideoDecoder::Impl::openFile(const QString& path) {
  ...
  if (avformat_open_input(&formatContext, path.toUtf8().constData(), nullptr, nullptr) < 0) {
    qWarning() << "FFmpegDecoder::Impl::openFile: Failed to open input file:" << path;
  ...
  if (videoStreamIndex == -1 || !codecParameters) {
...
DecodedVideoFrame FFmpegVideoDecoder::decodeNextVideoFrameRaw() {
  if (!impl_) {
    return std::monostate{};
  }
  return impl_->decodeNextVideoFrameRaw();
}
