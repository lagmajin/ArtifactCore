...
GlyphRect GlyphAtlas::acquire(const GlyphKey& key, const QFont& font)
{
    ...
    QImage glyphBitmap   = rawFont.alphaMapForGlyph(gindex, QRawFont::PixelAntialiasing);

    // アルファマップが空またはサポートされないコードポイント
    if (glyphBitmap.isNull() || glyphBitmap.width() <= 0 || glyphBitmap.height() <= 0) {
        GlyphRect empty;
        empty.valid = false;
        cache_[key] = empty;
        return empty;
    ...
    const QRectF br = rawFont.boundingRect(gindex, 1.0);
    ...
    rect.advance  = static_cast<float>(rawFont.advancesForGlyphIndexes({gindex}).value(0, QPointF()).x());
    rect.valid    = true;
