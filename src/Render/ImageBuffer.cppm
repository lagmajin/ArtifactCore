...
void ImageBuffer::setPixel(int x, int y, const Color& color, int samplesPerPixel)
{
    ...
    int idx = (y * width + x) * 3;
    pixels[idx + 0] = static_cast<std::uint8_t>(std::clamp(std::sqrt(safeChannel(color.x)) * scale, 0.0f, 0.999f) * 256);
    pixels[idx + 1] = static_cast<std::uint8_t>(std::clamp(std::sqrt(safeChannel(color.y)) * scale, 0.0f, 0.999f) * 256);
    pixels[idx + 2] = static_cast<std::uint8_t>(std::clamp(std::sqrt(safeChannel(color.z)) * scale, 0.0f, 0.999f) * 256);
}

bool ImageBuffer::savePNG(const char* filename) const
{
    if (!filename || pixels.empty() || width <= 0 || height <= 0)
        return false;
