...
LayerBlendPipeline::~LayerBlendPipeline() { delete pImpl_; }

bool LayerBlendPipeline::initialize() {
  if (!createConstantBuffer())
    return false;
  // [PERF]
  // 初期化時に全ブレンドモードをコンパイルせず、初回使用時に遅延コンパイルする
  // if (!createExecutors()) return false;
  qDebug() << "[LayerBlendPipeline] Initialized (lazy compilation enabled)";
  return true;
}
...
bool LayerBlendPipeline::createExecutors() {
  ...
  for (const auto &[mode, shaderCode] : BlendShaders) {
    ...
    if (!entry.executor->build(desc)) {
      qWarning() << "[LayerBlendPipeline] PSO build FAILED for blend mode"
    ...
    if (pImpl_->pBlendCB_) {
      ...
      qWarning() << "[LayerBlendPipeline] pBlendCB_ is null for blend mode"
                 << static_cast<int>(mode) << "- opacity will not work";
    }

    if (!entry.executor->createShaderResourceBinding(true)) {
      qWarning() << "[LayerBlendPipeline] SRB creation FAILED for blend mode"
                 << static_cast<int>(mode);
      ++failCount;
      continue;
    }
  ...
  qDebug() << "[LayerBlendPipeline] createExecutors done:" << successCount
           << "succeeded," << failCount << "failed"
...
bool LayerBlendPipeline::blend(IDeviceContext *ctx, ITextureView *srcSRV,
  ...
  if (!srcSRV) {
    qCritical() << "[LayerBlendPipeline::blend] srcSRV is null for mode"
                << static_cast<int>(mode);
    return false;
  }
  if (!dstSRV) {
    qCritical() << "[LayerBlendPipeline::blend] dstSRV is null for mode"
                << static_cast<int>(mode);
    return false;
  }
  if (!outUAV) {
    qCritical() << "[LayerBlendPipeline::blend] outUAV is null for mode"
                << static_cast<int>(mode);
    return false;
  }
  if (!pImpl_->pBlendCB_) {
    qCritical() << "[LayerBlendPipeline::blend] pBlendCB_ is null - constant "
  ...
  if (it == executors_.end()) {
    qWarning() << "[LayerBlendPipeline::blend] No executor for mode"
               << static_cast<int>(mode) << "- falling back to Normal";
    it = executors_.find(BlendMode::Normal);
    if (it == executors_.end()) {
      qCritical() << "[LayerBlendPipeline::blend] No Normal fallback executor "
                     "available!";
      return false;
    }
  }
  ...
  if (!exec.ready()) {
    qCritical() << "[LayerBlendPipeline::blend] executor not ready for mode"
                << static_cast<int>(mode);
    return false;
  }
  ...
  if (pData) {
    ...
    qWarning() << "[LayerBlendPipeline::blend] MapBuffer failed - opacity may "
                  "be wrong";
  }
...
bool LayerBlendPipeline::blendDirect(IDeviceContext *ctx, ITextureView *srcSRV,
  ...
  exec.setTextureView("SrcTex", srcSRV);
  // DstTex is required by the shader even in direct mode (though it might not
  // be used by all modes) For direct mode, we assume dst is not needed or we
  // use a dummy. Actually, the shaders I saw use DstTex.
  exec.setTextureView(
      "DstTex", srcSRV); // Fallback to src if not provided? No, that's wrong.
