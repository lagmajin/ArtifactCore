...
export struct TaskId {
    ...
    static auto Invalid() -> TaskId {
        return TaskId{UINT64_MAX};
    }
...
export struct TaskProgress {
    ...
    QString message;           // ユーザー向けメッセージ（例: "フレーム 120/300 を処理中..."）
...
export struct TaskError {
    ...
    static auto None() -> TaskError {
        return TaskError{};
    }
    
    static auto FromException(const QString& msg, const QString& cat = "unknown") -> TaskError {
        return TaskError{
            .code = -1,
            .message = msg,
            .details = "",
...
export struct TaskSnapshot {
    ...
    auto GetElapsed() const -> std::chrono::milliseconds {
        ...
        auto end = (state == TaskState::Running) ? std::chrono::steady_clock::now() : endTime;
...
public:
    ...
    /// </summary>
    virtual auto Execute(CancelToken& cancelToken, std::function<void(TaskProgress)> reportProgress) -> void = 0;
    
    /// <summary>
    /// Taskのオプションを返す
    /// </summary>
    virtual auto GetOptions() const -> TaskOptions {
        return TaskOptions{};
    }
    ...
    /// Taskの状態スナップショットを返す
    /// </summary>
    auto GetSnapshot() const -> TaskSnapshot {
        return snapshot_;
    }
