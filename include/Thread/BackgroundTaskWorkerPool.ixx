...
public:
    struct Config {
        int maxWorkers = std::thread::hardware_concurrency();  // デフォルトは論理コア数
    ...
    explicit BackgroundTaskWorkerPool(Config config = {})
        ...
        if (config_.maxWorkers <= 0) {
            config_.maxWorkers = std::max(1, static_cast<int>(std::thread::hardware_concurrency()));
        }
    }
    ...
    void Start() {
        ...
        // Scheduler threadを起動
        schedulerThread_ = std::thread(&BackgroundTaskWorkerPool::SchedulerLoop, this);
    }
...
private:
    ...
    void ExecuteTask(std::shared_ptr<IBackgroundTask> task, int workerId) {
        ...
        {
            std::lock_guard<std::mutex> lock(cancelMutex_);
            cancelTokens_[taskId] = cancelToken;
        }
        ...
        TaskSnapshot snapshot{
            ...
            .category = task->GetOptions().priority != TaskPriority::Idle ? 
                       TaskCategory::Custom : TaskCategory::Custom,
        ...
        // EventBusに通知
        TaskStateChangedEvent stateEvent{
            .taskId = taskId,
            .oldState = oldState,
            .newState = newState,
            .snapshot = snapshot
        ...
        // 状態変更イベント
        TaskStateChangedEvent finalStateEvent{
            .taskId = taskId,
            .oldState = oldState,
            .newState = newState,
            .snapshot = snapshot
