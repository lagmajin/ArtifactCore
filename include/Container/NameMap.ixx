...
export module Container.NameMap;
...
public:
  ...
  explicit NameMap(ContainerName name) noexcept
    : name_(name)
  {
  }

  NameMap(ContainerName name, ContainerSourceLocation createdAt) noexcept
    : name_(name)
    , createdAt_(createdAt)
  {
  }

  NameMap(ContainerName name, ContainerDomain domain) noexcept
    : name_(name)
    , domain_(domain)
  {
  }

  NameMap(ContainerName name, ContainerDomain domain, ContainerSourceLocation createdAt) noexcept
    : name_(name)
    , domain_(domain)
    , createdAt_(createdAt)
  {
  ...
  ContainerDebugInfo debugInfo() const noexcept
  {
    return ContainerDebugInfo{
      ...
      values_.size(),
      values_.size(),
      values_.size() * (sizeof(K) + sizeof(V))
    };
  }
  ...
  {
    NamedVector<ContainerElementSample> samples{ContainerName{"NameMapElementSamples"}};
    std::size_t index = 0;
    for (const auto& [key, value] : values_) {
  ...
  bool watch(const ContainerWatchRule& rule, Callback&& callback) const
  {
    ...
    if (hit) {
      callback(ContainerWatchHit{"namemap-watch", createdAt_, snapshot});
    }
    return hit;
  }
...
template <typename K, typename V>
NameMap<K, V> makeNameMap(ContainerName name)
{
  return NameMap<K, V>(name);
}

template <typename K, typename V>
NameMap<K, V> makeNameMap(ContainerName name, ContainerSourceLocation location)
{
  return NameMap<K, V>(name, location);
}

template <typename K, typename V>
NameMap<K, V> makeNameMap(const char* name)
{
  return NameMap<K, V>(ContainerName{name});
}

template <typename K, typename V>
NameMap<K, V> makeNameMap(const char* name, ContainerSourceLocation location)
{
  return NameMap<K, V>(ContainerName{name}, location);
}
